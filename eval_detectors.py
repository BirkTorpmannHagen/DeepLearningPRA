...
def collect_debiased_data(testbed_constructor, dataset_name, mode="noise", sampler="RandomSampler", k=5, batch_size=8):
    ...
    features = [cross_entropy, energy, softmax, typicality, grad_magnitude]
    ...
    if (uncollected_features== []):
        ...
        print(f"Collecting data for {dataset_name} in {mode} mode with {sampler} sampler and batch size {batch_size} and k={k} and features {', '.join([f.__name__ for f in features])}")
