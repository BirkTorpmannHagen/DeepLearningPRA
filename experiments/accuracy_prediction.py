            ...
            for shift in for_dataset["Shift"].unique():
                ...
                for ood_folds in test_organic["fold"].unique():
                    ...
                    for intensity in test_ood["shift_intensity"].unique():
                        ...
                        if test_ood.empty:
                            print("Empty for", dataset, feature_name, shift, intensity)
                            input()
                        ...
                        for proportion in np.linspace(0,1,11):
                            ...
                            pred = gam.predict(detection_rate)[0]
                            mae = np.abs(gap - pred)
                            baseline =np.abs(gap - 0)
                            gam_data.append({"Dataset":dataset, "dr":detection_rate, "gap":gap,"prediction":pred, "proportion":proportion, "intensity":intensity, "feature_name":feature_name, "Shift":shift,
                                             "mae":mae, "naive baseline mae": baseline})

        gam_df = pd.DataFrame(gam_data)
        gam_df.to_csv(f"acc_estimation_data/{dataset}_acc_prediction_gam_results.csv", index=False)
...
def test_generalization_gap_estimation(batch_size):
    ...
    for dataset in DATASETS:
        ...
        for shift in for_dataset["Shift"].unique():
            ...
            print(f"{dataset:<15} {shift:<20} {mae:>10.2f} {baseline:>10.2f}")
    ...
    gam_df = pd.DataFrame(gam_data)
    gam_df.to_csv("gam_fits.csv", index=False)
